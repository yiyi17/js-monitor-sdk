 ...
 */
function _has(object: any[] | HTMLCollectionOf<HTMLScriptElement>, key: PropertyKey) {
 ...
 */
function _isUndefined(what: string | Document) {
 ...
 */
// @ts-ignore
TraceKit.wrap = function traceKitWrapper(func: { apply: (arg0: any, arg1: IArguments) => any }) {
  function wrapped() {
    try {
      // @ts-ignore
...
TraceKit.report = (function reportModuleWrapper() {
  ...
  let _oldOnerrorHandler: OnErrorEventHandler, _onErrorHandlerInstalled: boolean;
...
TraceKit.computeStackTrace = (function computeStackTraceWrapper() {
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function getSource(url: PropertyKey) {
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function guessFunctionName(url: string | null, lineNo: number | null) {
    // eslint-disable-next-line prefer-const
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function gatherContext(url: string | null, line: number) {
    // @ts-ignore
    ...
    for (let i = start; i < end; ++i) {
      if (!_isUndefined(source[i])) {
        context.push(source[i].length < 100 ? source[i] : source[i].substr(0, 100) + '...');
      }
    }
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function escapeCodeAsRegExpForMatchingInsideHTML(body: string) {
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function findSourceInUrls(re: RegExp, urls: string | any[]) {
    let source, m;
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function findSourceInLine(fragment: string, url: string | null, line: number) {
    // @ts-ignore
    // eslint-disable-next-line prefer-const
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function findSourceByFunctionBody(func: string | Function) {
  ...
  function computeStackTraceFromStackProp(ex: {
    stack: string;
    message: string;
    columnNumber: string | number | Document;
    name: any;
  }) {
    if (!ex.stack) {
      return null;
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function computeStackTraceFromStacktraceProp(ex: { stacktrace: any; name: any; message: any }) {
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function computeStackTraceFromOperaMultiLineMessage(ex: { message: string; name: any }) {
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function computeStackTrace(ex: unknown, depth: number | null) {
    let stack = null;
   ...
   * @memberof TraceKit.computeStackTrace
   */
  function computeStackTraceOfCaller(depth: number | null) {
...
TraceKit.extendToAsynchronousCallbacks = function () {
  const _helper = function _helper(fnName: string) {
    // @ts-ignore
